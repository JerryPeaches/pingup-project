...
import { Image, ImageIcon, SendHorizonal, SendHorizontal } from 'lucide-react'
  ...
  return user && (
    <div className='flex flex-col h-screen'>
      ...
      <div className='p-5 md:px-10 h-full overflow-y-scroll'>
        <div className='space-y-4 max-w-4cl mx-auto'>
            {
      ...
      <div className='p-x-4'>
            <div className='flex items-center gap-3 pl-5 p-1.5 bg-white w-full max-w-xl mx-auto border border-gray-200 shadow rounded-full mb-5'>
                ...
                <label htmlFor="image">
                    {
                        image 
                        ? <img src={URL.createObjectURL(image)} alt="" className='h-4 rounded'/> 
                        : <ImageIcon className='size-7 text-gray-400 cursor-pointer'/>
                    }
                    <input type="file" id='image' accept='image/*' hidden onChange={(e)=>setImage(e.target.files[0])}/>
                </label>
                <button onClick={sendMessage} className='bg-gradient-to-br from-indigo-500 to-purple-600 hover:from-indigo-700 hover:to-purple-800 hover:scale-105 active:scale-95 cursor-pointer text-white p-2 rounded-full'>
